...
namespace FreeHubProject
{
    public partial class Notifications : System.Web.UI.Page
    {
        ...
        private string CurrentFilter
        {
            get
            {
                return Session["NotificationFilter"] != null ? Session["NotificationFilter"].ToString() : "All";
            }
            set
            {
                Session["NotificationFilter"] = value;
            }
        }
            ...
            using (SqlConnection conn = new SqlConnection(_connStr))
            {
                string query = @"
                    SELECT 
                        ...
                        content AS Preview,
                        FORMAT(notificationTimestamp, 'yyyy-MM-dd HH:mm') AS Time,
                        status AS Status
        ...
        private void UpdateTabBadgesAndStyles()
        {
            ...
            btnAll.Text = allCount > 0 ? $"All ({allCount})" : "All";
            btnUnread.Text = unreadCount > 0 ? $"Unread ({unreadCount})" : "Unread";
            btnArchived.Text = archivedCount > 0 ? $"Archived ({archivedCount})" : "Archived";
            ...
            switch (CurrentFilter)
            {
                case "Unread":
                    ...
                    break;
                case "Archived":
                    btnArchived.CssClass = "notification-tab active-tab";
                    break;
                default:
        ...
        private void LoadSelectedNotification(int notificationId)
        {
            ...
            {
                string query = @"
                    SELECT notificationID, type, content, FORMAT(notificationTimestamp, 'yyyy-MM-dd HH:mm') AS notificationTimestamp, status 
                    FROM dbo.Notification 
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                        {
                            ...
                            // DYNAMICALLY TOGGLE READ/UNREAD BUTTON TEXT
                            if (status == "Read")
                            {
                                btnToggleRead.Text = "Mark as Unread";
                            }
                            else
                            {
                                btnToggleRead.Text = "Mark as Read";
                            }

                            // DYNAMICALLY TOGGLE ARCHIVE/UNARCHIVE BUTTON TEXT
                            if (status == "Archived")
                            {
                                btnToggleArchive.Text = "Unarchive";
                            }
                            else
                            {
                                btnToggleArchive.Text = "Archive";
                            }
        ...
        protected void btnToggleArchive_Click(object sender, EventArgs e)
        {
            ...
            {
                // Restore to 'Read' when unarchived
                UpdateNotificationStatus(notificationId, "Read");
                pnlSelectedNotification.Visible = false;
                lblSelectNotification.Visible = true;
